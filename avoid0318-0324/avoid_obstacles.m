...
%     rectangle('Position',[X_obstacle(1),Y_obstacle(1),5,0.5],'FaceColor',[0 0 0]);
    ...
    for i = 1:numel(X_obstacle)
        x = X_obstacle(i) + dX_obstacle(i)*t;
        y = Y_obstacle(i) + dY_obstacle(i)*t;
        rectangle('Position', [x-radius, y-radius, 2*radius, 2*radius], 'Curvature', [1, 1], ...
            'FaceColor', [obs_color, obs_color, obs_color], 'EdgeColor', [obs_color, obs_color, obs_color]);
    end
...
% 以下根据计算出的控制量推导所有的状态量
 ...
 for i=1:1:Np
    ...
    plot(xx,yy);
    plot(X,Y,'Color', [obs_color, obs_color, obs_color],'Marker', 'o');
    hold on;
    toc
% End of mdlOutputs.
 %% 求代价函数的功能子函数
function cost = MY_costfunction(x,State_Initial,Np,Nc,Nobs,T,Yref,Q,R,S,X_obstacle,Y_obstacle,dX_obstacle,dY_obstacle,phi,t)
    ...
    for i=1:1:Np
        if i==Nc-1 
            ...
            for j=1:1:Nobs
                delta_x = X_obstacle(j,1) + t*dX_obstacle(j,1) + i*T*dX_obstacle(j,1) - X_predict(i,1);
                delta_y = Y_obstacle(j,1) + t*dY_obstacle(j,1) + i*T*dY_obstacle(j,1) - Y_predict(i,1);
...
%                 end
            ...
            for p=1:1:Nobs
                delta_x = X_obstacle(p,1) + t*dX_obstacle(p,1) + i*T*dX_obstacle(p,1) - X_predict(i,1);
                delta_y = Y_obstacle(p,1) + t*dY_obstacle(p,1) + i*T*dY_obstacle(p,1) - Y_predict(i,1);
                dx_obs = delta_x*cos(phi) + delta_y*sin(phi);
                dy_obs = delta_x*sin(phi) - delta_y*cos(phi);
%                 d = 1000000000;
%                 if dy_obs > -c && dy_obs < c 
