...
ddXbc  = Fx/pusr.mt;
ddYbc  = Fy/pusr.mt;
ddXb       = Fx/pusr.mt ; % +dYaw.*dYb; % eq1
ddYb       = Fy/pusr.mt ; % -dYaw.*dXb; % eq2
ddYaw      = Tyaw/pusr.Jzz; % eq3
ddomega1    = (Tw1-Ftxfr.*pusr.Rt(1))./pusr.Jwy(1); % eq4
ddomega2    = (Tw2-Ftxfl.*pusr.Rt(2))./pusr.Jwy(2); % eq5
ddomega3    = (Tw3-Ftxrr.*pusr.Rt(3))./pusr.Jwy(3); % eq6
ddomega4    = (Tw4-Ftxrl.*pusr.Rt(4))./pusr.Jwy(4); % eq7

dgXb       = dXb.*cos(Yaw)-dYb.*sin(Yaw); %  % eq8
dgYb       = dXb.*sin(Yaw)+dYb.*cos(Yaw); % % eq9
dgYaw   = dYaw; % eq10
Xout       = [ddXb; ddYb; ddYaw; ddomega1; ddomega2; ddomega3; ddomega4; dgXb; dgYb; dgYaw];


vars = [Tw1; Tw2; Tw3; Tw4; steer]; 
...
matlabFunction(A0, 'file', 'genA_7dof_0316', 'vars', vars_need);
